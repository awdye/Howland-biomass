...
#aggregate individual values by year with a mean values of GPP and RE for each year
tower.data.agg<-aggregate(tower.data,list(tower.data$Year, tower.data$JD), mean)
tower.data.aggr<-aggregate(tower.data,list(year_mean=tower.data$Year), sum)
tower.data$GPPsec<-tower.data$GPP*1800
tower.data$REsec<-tower.data$RE*1800
tower.data.GPP<-aggregate(tower.data$GPPsec, list(year_sum=tower.data$Year), sum)
setnames(tower.data.GPP, "x", "GPP")
tower.data.RE<-aggregate(tower.data$REsec, list(year_sum=tower.data$Year), sum)
setnames(tower.data.RE, "x", "RE")
tower.data.GPP$RE<-tower.data.RE$RE
tower.data.GPP$NPP<-tower.data.GPP$GPP-tower.data.GPP$RE

tower.data.GPP$GPPkgha<-tower.data.GPP$GPP/1000000*0.0120107*10000
tower.data.GPP$REkgha<-tower.data.GPP$RE/1000000*0.0120107*10000
tower.data.GPP$NPPkgha<-tower.data.GPP$NPP/1000000*0.0120107*10000
tower.data.GPP=tower.data.GPP[-10,]
...
plot(ab.ha.data$total.ha,tower.data.GPP$NPPkgha)
cor.test(ab.ha.data$total.ha,tower.data.GPP$NPPkgha, use="complete.obs", method="pearson")
